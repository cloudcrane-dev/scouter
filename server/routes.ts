...
import { sendInsightNotification, sendTopStrengthEmail, sendTopViewsEmail, sendLowStrengthEmail, getISOWeekKey } from "./email";
...
export async function registerRoutes(
  ...
  app.post("/api/admin/send-rank-emails", async (req, res) => {
    try {
      ...
      const adminSecret = process.env.ADMIN_SECRET || "skillsniffer-admin";
      if (secret !== adminSecret) return res.status(403).json({ error: "Forbidden" });

      const weekKey = getISOWeekKey();
      const results = { topStrength: 0, topViews: 0, lowStrength: 0, skipped: 0, errors: [] as string[] };
      ...
      res.json({ success: true, weekKey, ...results });
    } catch (error) {
